...
1. 安装扩展
   ...
   - 点击「加载已解压的扩展程序」，选择本仓库中的 `extension/` 目录。

2. 配置追踪标的与显示
   - 在扩展卡片点击「详情 → 扩展选项」，或直接打开选项页。
   - 填写 `股票代码`（示例：`sh000300`、`sz000001`）。需符合所选数据源的代码格式（沪市 `sh` 前缀、深市 `sz` 前缀）。
   - 选择 `行情源`（腾讯/新浪）与合适的 `刷新间隔`（建议 ≥ 3 秒）。
   - 选择 `显示内容`（价格、涨跌幅或二者），并根据需要调整 `气泡尺寸`、`透明度`、`主题`。
   - 点击「保存配置」。后台将在下一个轮询周期自动刷新。

3. 页面内查看与操作
   - 打开任意网页即可看到悬浮行情气泡。
   - 拖拽气泡的「标题栏」可移动位置；支持「折叠/展开」、「立即刷新」与「隐藏」。
   - 若已隐藏，页面左下角会出现「显示行情气泡」按钮，点击可恢复显示。

4. 重置气泡状态
   - 如需恢复默认位置/尺寸/折叠状态，前往选项页点击「重置气泡位置」。

5. 常见示例
   - 跟踪沪深指数：`sh000300`（沪深300）、`sh000001`（上证指数）、`sz399001`（深证成指）。
   - 跟踪个股：`sh600000`、`sz000001` 等，注意前缀与交易所匹配。
